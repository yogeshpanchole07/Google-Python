...
# When you display approved_users[0] and approved_devices[0], the output shows the first username ("elarson") and its corresponding device ID ("8rp2k75").
# 
# If you change the index to another number (like 1), the output updates to the next pair ("bmoreno" and "hl0s5o1"). This shows the two lists are synchronized, meaning the data at each index is directly related.
...
# [Double-click to enter your responses here.]When both lists are displayed, the new username ("gesparza") and the new device ID ("3rcv4w6") appear at the very end of their respective lists. This confirms that the .append() method successfully added the new data while maintaining the synchronization between the two lists.
...
# When the lists are displayed, the username "tshah" and the device ID "2ye3lzg" have been removed. The lists remain synchronized because the .remove() method deleted the specific elements at their respective positions, leaving the remaining users correctly paired with their assigned devices.
...
# The output displays the message: "The username sgilmore is approved to access the system." This occurs because the if statement evaluates to True since "sgilmore" is present in the approved_users list.
...
# The output is 2. This represents the index position of "sgilmore" in the approved_users list. Since Python uses zero-based indexing, "sgilmore" is the third item in the list (0, 1, 2).
...
# The output is "4n482ts". This is the device ID assigned to "sgilmore". By finding the index of the username in the approved_users list and applying that same index (ind) to the approved_devices list, the algorithm successfully retrieves the specific device associated with that user.
...
# When the username is "sgilmore" and the device_id is "4n482ts", the output displays both success messages:
# 
# "The username sgilmore is approved to access the system."
# 
# "4n482ts is the assigned device for sgilmore"
# 
# Observation: This happens because both conditions in the if statement are True. The user is in the approved list, and the device ID provided matches the one stored at the corresponding index in the device list.
...
elif username in approved_user and device_id != approved_devices[ind]:
...
# When username is "sgilmore" and device_id is "4n482ts", the code still triggers the first if statement because the information is correct.
# 
# Observation: If you were to change device_id to something else (like "wrong_id"), the elif block would execute. It would inform the user that while their username is recognized, the device they are using does not match the one recorded at their specific index in the system.
...
login("sgilmore", "4n482ts")  # Correct user, correct device
login("sgilmore", "wrong_id") # Correct user, wrong device
login("unknown_user", "12345") # Unapproved user
...
# When the device_id is correct: The inner conditional evaluates to True, and the code displays a success message confirming the device is assigned to that user.
# 
# When the device_id is incorrect: The else block of the inner conditional executes, informing the user that while their username is approved, the device ID they provided does not match the one on file.

# ## Conclusion
# 
# **What are your key takeaways from this lab?**

# Synchronization is Key: Keeping two lists synchronized by index allows you to link related data (like users and devices) without using more complex structures like dictionaries yet.
# 
# Nested Logic: Using nested if statements allows for a more granular security check—first verifying identity, then verifying the specific hardware authorization.
# 
# Efficiency with Functions: Defining a login function makes the code reusable and cleaner, allowing you to check any user/device pair with a single line of code.
