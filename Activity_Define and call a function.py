...
# The `alert()` function is a reusable block of code that prints the message **"Potential security issue. Investigate further."** to the console. If called, the output would be that specific text string.
...
# Reusability: You can call alert() multiple times throughout your program without rewriting the print() statement.
# 
# Maintainability: If you need to change the alert message later, you only have to update it in one place (the function definition).
# 
# Readability: It makes the main part of your code cleaner and easier to understand by using a descriptive name for a specific action.
...
# The Previous Version: Contained a simple print() statement that executed a single time when the function was called.
# 
# The New Version: Incorporates a for loop using range(3). This causes the code block inside the function to iterate three times, resulting in the repeated output.
...
# The output shows each username from the username_list printed on a new line. This happens because the for loop iterates through the list one by one, and the print() function automatically adds a newline character after each execution. While the data is all there, it is currently formatted as a vertical list rather than a single
...
# The output shows all the usernames combined into a single, continuous string (elarsonbmorenotshah...). Because the code used string concatenation (sum_variable + i) without adding any spaces or punctuation between the elements, the usernames run into each other, making the text difficult to read.
# 
# While the data is now consolidated into one variable, it lacks the formatting needed for clear separation.
...
# The output is now much more readable. By adding a string containing a comma and a space (", ") during the concatenation process, the usernames are clearly separated (e.g., elarson, bmoreno, tshah...). This formatting allows a security analyst to easily distinguish between individual users within the single string.

# ## Conclusion
# 
# **What are your key takeaways from this lab?**

# Automation through Functions: I learned how to define and call functions to automate repetitive tasks, such as displaying security alerts.
# 
# Code Efficiency: Using functions improves code reusability and maintainability, allowing for global updates by changing only a single block of code.
# 
# Data Transformation: I practiced converting data types (from a list to a string) using for loops and string concatenation, which is essential for tasks like writing logs to text files.
# 
# Readability Matters: I discovered that using delimiters (like commas and spaces) during concatenation is critical for ensuring that automated outputs remain human-readable for security analysis.
