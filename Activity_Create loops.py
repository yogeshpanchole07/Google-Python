...
# The for loop is count-controlled and handles the counter automatically, making it best for a fixed number of repeats. The while loop is condition-controlled, requiring manual variable management (initialization and incrementing), but offers more flexibility for unknown iteration counts.
...
# The print(i) statement is placed outside and before the conditional because we want to display every employee ID generated by the loop. If it were inside the if i == 5100: block, the program would only print the number 5100 and skip displaying all the other IDs

# ## Conclusion
# **What are your key takeaways from this lab?**
# 
# Loop Selection: for loops are ideal when the number of iterations is known (like checking a specific list of IP addresses), while while loops are better for iterating until a specific security condition is met.
# 
# Automation: Using iterative statements allows a security analyst to efficiently process large datasets, such as checking login attempts against an allow list, which would be impossible to do manually.
# 
# Flow Control: The break keyword is a powerful tool for stopping a process immediately when a potential threat is detected (like an unauthorized IP), preventing further execution of the script.
# 
# Conditional Logic: Nesting if statements within loops provides a way to create automated alerts and decision-making logic within a script.    
# 
# 
# 
# 
