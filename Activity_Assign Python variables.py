...
# we printed the value of device id by the help of assoigning value an then then printed the type of the value
...
# The data type of username_list is a list. This is indicated by the square brackets used during assignment and confirmed by the <class 'list'> output in Task 4. It allows you to store multiple string values in a single, ordered variable
...
# Based on the output in Task 5, the contents of username_list have been updated to include a new string. While the original list contained five usernames, the new output shows a sixth username, "lpope", has been successfully added to the end of the collection.
...
# The output is **True**. This means that the current number of login attempts (`2`) is less than or equal to the maximum number of attempts allowed (`3`).true
...
# Based on the different values assigned, I observed that the output is a **Boolean** (`True` or `False`). It returns `True` when `login_attempts` is 3 or less, and switches to `False` once the attempts exceed the `max_logins` threshold.
...
# Based on the output of Task 10, the data type of `login_status` is a **Boolean**. This is indicated by the `<class 'bool'>` output, which Python uses to represent one of two possible values: `True` or `False`.

# ## Conclusion
# 
# **What are your key takeaways from this lab?**

# Variable Assignment: Python uses the = operator to store data (like strings, integers, and lists) in descriptive variable names for later use.
# 
# Data Types: Different types of security data require specific formats, such as str for IDs, list for collections of users, int for counts, and bool for status checks.
# 
# Dynamic Updating: Variables in Python are flexible, allowing you to reassign values or update list contents as information (like a user allow list) changes.
