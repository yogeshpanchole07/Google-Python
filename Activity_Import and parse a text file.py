...
# Before .split(): The output is one continuous string. It includes newline characters (like \n) that format the text into lines when printed, but Python treats it as a single, large block of data.
# 
# After .split(): The output becomes a list of strings. Each line from the original log file is now an individual element (a separate string) within that list, enclosed in brackets [] and separated by commas.
...
# The new entry was added to the last line of the file. Because the original file didn't end with a newline character, the jrafael entry appears immediately after the previous last entry (jsoto...).
...
# Context Managers (with statement): Using with is the standard best practice because it ensures the file is closed automatically, preventing data corruption or memory leaks.
# 
# File Modes: Choosing the right mode is critical:
# 
# "r" (Read) is for extracting data.
# 
# "a" (Append) is for adding new data to the end of a file without deleting existing content.
# 
# "w" (Write) is for creating a new file or completely overwriting an old one.
# 
# Parsing with .split(): Security logs often come as one giant string. Using .split() is essential to convert that string into a list, allowing you to iterate through specific records (like individual IP addresses) more efficiently.
# 
# Method Differences: Understanding that .read() pulls data out while .write() pushes data in is the foundation of automated log management.
