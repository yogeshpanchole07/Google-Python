...
# Regex Pattern: r15\w+
# r15: Matches the literal characters "r15". If missing, the code would find all device IDs (e.g., r262c36) instead of just those needing updates.
# 
# \w: Matches any alphanumeric character. If missing, the code would only look for the literal string "r15" and ignore the rest of the ID.
# 
# +: Matches one or more occurrences. If missing, the code would only return "r15" plus a single character (e.g., r151) instead of the entire ID (e.g., r151dm4).
...
# Extracted Examples: 192.168.152.148 and 192.168.190.178. These matched because they have exactly three digits per segment.
# 
# Missed Examples: 192.168.22.115 and 192.168.96.200. These were missed because segments like .22. and .96. only have two digits.
# 
# Validity: The missed addresses are valid. IP segments can legally range from one to three digits (0-255). The current pattern is too restrictive.
...
# Improved Precision: By using \d{1,3}, the pattern now correctly identifies valid IP segments that have one, two, or three digits (like the .22. and .96. segments that were missed in Task 7).
# 
# Effective Filtering: Unlike Task 8 (which used \d+), this pattern successfully excludes invalid IP addresses. For example, it will no longer extract "19245" or "2345" as valid segments because they exceed the three-digit limit.
# 
# Result: The list valid_ip_addresses now contains only the strings that strictly follow the structure of a standard IPv4 address.
...
# Efficiency through Automation: Using the re module allows for the rapid extraction of specific data (like device IDs and IP addresses) from large, unstructured text blocks that would be nearly impossible to sort manually.
# 
# Precision Matters: I learned that basic regex patterns (like \d+) can be too broad and capture invalid data. Using more specific quantifiers (like {1,3}) ensures that the extracted data adheres to actual technical standards, such as IPv4 formatting.
# 
# Data Validation: Regular expressions are a powerful first step in a security pipeline. Once the data is cleaned and extracted, Python’s control flow (loops and conditionals) makes it easy to cross-reference that data against threat intelligence lists to identify suspicious activity.
