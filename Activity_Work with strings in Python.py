...
# in the first it was an integer and then after applying str function to employee_id it is converted to string and after run this print(type(employee_id)) output shows str type
...
# This code outputs .com.It works because the ind variable stores the starting index of the substring ".com" (which is 19). The slice [ind:ind+4] tells Python to start at index 19 and stop at index 23 ($19 + 4 = 23$). Since the "stop" index is exclusive, Python grabs the four characters at indices 19, 20, 21, and 22, which perfectly capture ., c, o, and m.
# 
# 
...
# 
# Key Takeaways
# Data Type Conversion: Learned how to use the str() function to convert integers to strings, which is necessary before performing string-specific operations like concatenation or length checks.
# 
# String Concatenation: Practiced using the + operator to join strings together (like adding an "E" prefix to an ID) to meet specific formatting standards.
# 
# Indexing and Slicing: Gained a solid understanding of Python's 0-based indexing. I practiced extracting specific characters using single indices and "sub-strings" using the [start:stop] slice syntax (remembering that the stop index is exclusive).
# 
# Dynamic Indexing with .index(): Learned how to use the .index() method to find the position of a substring (like .com) programmatically. This makes code more flexible because it can handle data of varying lengths without needing to hard-code specific numbers.
# 
#     
# 
# 
