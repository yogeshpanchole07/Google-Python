...
# after applying sorted function in print function give you rhe sorted output in ascending order
...
# after applying max fuction in print function gives me the highest failed login attempts which was happened in november month
...
# function displayes current day login for ejoned is 9 .Yes, the output is dynamic. Because the function uses parameters, the message will change depending on the arguments you pass into it. If you called it with analyze_logins("asmith", 12), the output would update to reflect those new values.
...
# his version of the analyze_logins() function displays three things:
# 
# The current day's total login attempts.
# 
# The average daily login attempts.
# 
# The calculated ratio showing how many times more the user logged in today compared to their average.
# 
# Would the output vary?
# Yes, the output is dynamic. Because the function uses parameters, the specific messages and the calculated ratio will change based on the arguments passed into the function call. For example, changing the current_day_logins or average_day_logins will result in a different login_ratio being displayed.
...
# Key Takeaways
# Built-in Functions: I learned how to use built-in functions like sorted() to organize data and max() to identify security anomalies (outliers) in a dataset.
# 
# User-Defined Functions: I practiced defining functions with multiple parameters (username, current_day_logins, etc.), which makes code reusable and dynamic for different users.
# 
# Data Analysis: I learned how to automate security calculations, such as finding the login ratio, to help identify suspicious activity more efficiently than manual review.
# 
# The Power of return: I observed that using the return keyword is essential for passing data out of a function so it can be stored in a variable and used in further logic, like a conditional statement for triggering alerts.
